...
public class HeavenTeleporter
{
	...
	public HeavenTeleporter(WorldServer par1WorldServer)
	{
		super();
			...
			for (int l = -2; l <= 2; ++l)
			{
				for (int i1 = -2; i1 <= 2; ++i1)
				{
					for (int j1 = -1; j1 < 3; ++j1)
					{
						...
						/** change this block **/
						this.worldServerInstance.setBlock(k1, l1, i2, (Block) (flag ? Blocks.diamond_block : 0));
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
	{
		...
		else
		{
			for (k1 = l - short1; k1 <= l + short1; ++k1)
			{
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
				{
					...
					{
						/** change this block **/
						if (this.worldServerInstance.getBlock(k1, i2, l1) == HHBlocks.heavenPortal)
						{
							/** change this block **/
							while (this.worldServerInstance.getBlock(k1, i2 - 1, l1) == HHBlocks.heavenPortal)
							{
								--i2;
							}
		...
		if (d3 >= 0.0D)
		{
			if (flag)
			{
				this.destinationCoordinateCache.add(j1, new CustomPortalPostion(this, i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			int j2 = -1;
			/** change this block **/
			if (this.worldServerInstance.getBlock(i - 1, j, k) == HHBlocks.heavenPortal)
			{
				j2 = 2;
			}
			/** change this block **/
			...
			/** change this block **/
			if (this.worldServerInstance.getBlock(i, j, k - 1) == HHBlocks.heavenPortal)
			{
				j2 = 3;
			}
			/** change this block **/
			...
			for (k2 = -1; k2 <= 1; ++k2)
			{
				for (i3 = 1; i3 < 3; ++i3)
				{
					for (l2 = -1; l2 < 3; ++l2)
					{
						...
						/** change this block **/
						this.worldServerInstance.setBlock(k3, j3, i4, (Block) (flag ? Blocks.diamond_block : 0));
					}
				}
			}
		}
		for (k2 = 0; k2 < 4; ++k2)
		{
			for (i3 = 0; i3 < 4; ++i3)
			{
				for (l2 = -1; l2 < 4; ++l2)
				{
					...
					/** change these blocks **/
					this.worldServerInstance.setBlock(k3, j3, i4, flag ? Blocks.diamond_block : HHBlocks.heavenPortal, 0, 2);
				}
			}
			for (i3 = 0; i3 < 4; ++i3)
			{
	...
	public void removeStalePortalLocations(long p_85189_1_)
	{
		...
		{
			Iterator iterator = (Iterator) this.destinationCoordinateKeys.iterator();
			long j = p_85189_1_ - 600L;

			while (iterator.hasNext())
			{
				Long olong = (Long)iterator.next();//gets error here
				PortalPosition portalposition = (HeavenTeleporter.PortalPosition)this.destinationCoordinateCache.getValueByKey(olong.longValue());
